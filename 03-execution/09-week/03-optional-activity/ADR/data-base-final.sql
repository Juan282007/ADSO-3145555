...
-- [ADR-002] Reemplaza CHECK inline en reservation_passenger
CREATE TABLE passenger_type (
    ...
    CONSTRAINT ck_passenger_type_ages  CHECK (
        max_age_years IS NULL OR min_age_years IS NULL
...
-- [ADR-001] Reemplaza CHECK inline en miles_transaction
CREATE TABLE transaction_type (
    transaction_type_id     uuid     PRIMARY KEY DEFAULT gen_random_uuid(),
    type_code               varchar(20) NOT NULL,
    type_name               varchar(80) NOT NULL,
    affects_balance_sign    smallint NOT NULL,
    created_at              timestamptz NOT NULL DEFAULT now(),
    updated_at              timestamptz NOT NULL DEFAULT now(),
    CONSTRAINT uq_transaction_type_code  UNIQUE (type_code),
    CONSTRAINT ck_transaction_type_sign  CHECK (affects_balance_sign IN (-1, 1))
);

CREATE TABLE miles_transaction (
    miles_transaction_id    uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    loyalty_account_id      uuid NOT NULL REFERENCES loyalty_account(loyalty_account_id),
    transaction_type_id     uuid NOT NULL REFERENCES transaction_type(transaction_type_id), -- [ADR-001]
...
CREATE TABLE reservation_passenger (
    ...
    person_id                   uuid NOT NULL REFERENCES person(person_id),
    passenger_type_id           uuid NOT NULL REFERENCES passenger_type(passenger_type_id), -- [ADR-002]
...
CREATE TABLE ticket_segment (
    ...
    CONSTRAINT uq_ticket_segment_pair     UNIQUE (ticket_segment_id, flight_segment_id), -- requerido por ADR-009
...
CREATE TABLE seat_assignment (
    ...
    CONSTRAINT fk_seat_assignment_ticket_segment FOREIGN KEY (ticket_segment_id, flight_segment_id)
        REFERENCES ticket_segment(ticket_segment_id, flight_segment_id) -- [ADR-009]
);

-- [ADR-003] Reemplaza CHECKs inline en baggage
...
CREATE TABLE baggage (
    ...
    baggage_type_id     uuid          NOT NULL REFERENCES baggage_type(baggage_type_id),   -- [ADR-003]
    baggage_status_id   uuid          NOT NULL REFERENCES baggage_status(baggage_status_id), -- [ADR-003]
...
CREATE TABLE invoice_line (
    ...
    -- line_total se omite intencionalmente (ver ADR-007: preservar 3FN)
    created_at          timestamptz   NOT NULL DEFAULT now(),
    updated_at          timestamptz   NOT NULL DEFAULT now(),
...
COMMENT ON TABLE seat_assignment   IS 'Asignacion de asiento normalizada por ticket_segment con control de unicidad por segmento y asiento. FK compuesta garantiza coherencia (ADR-009).';
COMMENT ON TABLE loyalty_account_tier IS 'Historial de asignacion de nivel para evitar dependencia transitiva en loyalty_account (ADR-008).';
COMMENT ON TABLE invoice_line      IS 'Detalle facturable sin totales derivados persistidos, para preservar 3FN (ADR-007).';
COMMENT ON TABLE transaction_type  IS 'Tabla de referencia para tipos de transaccion de millas. Reemplaza CHECK inline (ADR-001).';
COMMENT ON TABLE passenger_type    IS 'Tabla de referencia para tipos de pasajero. Reemplaza CHECK inline (ADR-002).';
COMMENT ON TABLE baggage_type      IS 'Tabla de referencia para tipos de equipaje. Reemplaza CHECK inline (ADR-003).';
COMMENT ON TABLE baggage_status    IS 'Tabla de referencia para estados de equipaje. Reemplaza CHECK inline (ADR-003).';

-- ============================================================
-- TRIGGER ADR-004: Coherencia fare ↔ rutas de flight_segment
-- ============================================================

CREATE OR REPLACE FUNCTION fn_validate_ticket_fare_route()
...
DECLARE
    ...
    v_seg_dest      uuid;
BEGIN
    SELECT f.origin_airport_id, f.destination_airport_id
    INTO   v_fare_origin, v_fare_dest
    FROM   ticket t
    JOIN   fare   f ON f.fare_id = t.fare_id
    WHERE  t.ticket_id = NEW.ticket_id;

    SELECT fs.origin_airport_id
    INTO   v_seg_origin
    ...
    ORDER  BY ts.segment_sequence_no ASC
    LIMIT  1;

    SELECT fs.destination_airport_id
    INTO   v_seg_dest
    FROM   ticket_segment  ts
    JOIN   flight_segment  fs ON fs.flight_segment_id = ts.flight_segment_id
    WHERE  ts.ticket_id = NEW.ticket_id
    ORDER  BY ts.segment_sequence_no DESC
    LIMIT  1;

    IF v_fare_origin <> v_seg_origin OR v_fare_dest <> v_seg_dest THEN
        RAISE EXCEPTION
            'La ruta de la tarifa no coincide con los segmentos del ticket %',
            NEW.ticket_id;
    END IF;
...
CREATE CONSTRAINT TRIGGER trg_ticket_fare_route
AFTER INSERT OR UPDATE ON ticket_segment
DEFERRABLE INITIALLY DEFERRED
FOR EACH ROW EXECUTE FUNCTION fn_validate_ticket_fare_route();

-- ============================================================
-- VISTAS DE CONVENIENCIA (recomendadas por ADR-007 y ADR-008)
-- ============================================================

CREATE VIEW v_loyalty_account_current_tier AS
SELECT DISTINCT ON (loyalty_account_id)
    loyalty_account_id,
    loyalty_tier_id,
    assigned_at,
    expires_at
FROM  loyalty_account_tier
ORDER BY loyalty_account_id, assigned_at DESC;

COMMENT ON VIEW v_loyalty_account_current_tier IS
    'Tier actual de cada cuenta de fidelizacion (ADR-008).';
...
CREATE INDEX idx_loyalty_account_tier_assigned_at ON loyalty_account_tier(loyalty_account_id, assigned_at DESC);
CREATE INDEX idx_miles_transaction_account_id      ON miles_transaction(loyalty_account_id);
CREATE INDEX idx_miles_transaction_type_id         ON miles_transaction(transaction_type_id);
...
CREATE INDEX idx_reservation_passenger_type_id     ON reservation_passenger(passenger_type_id);
...
CREATE INDEX idx_baggage_type_id                   ON baggage(baggage_type_id);
CREATE INDEX idx_baggage_status_id                 ON baggage(baggage_status_id);
